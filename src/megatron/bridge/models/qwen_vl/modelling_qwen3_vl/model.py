...
class Qwen3VLModel(MegatronModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # A training framework may pre-shard input_ids across CP ranks (THD packed,
        # zigzag layout), detected by cu_seqlens_q[-1] == cp_size * len(input_ids).
        # In that case the model must NOT partition/slice again; vision-embed rows are
        # instead selected via _cp_local_vision_embed_indices below.
        packed_input_pre_sharded = False
        if (
            ...
            packed_input_pre_sharded = int(_cu[0]) == 0 and int(_cu[-1]) == cp_size * input_ids.numel()
        packed_cp_index = (
        ...
        if self.pre_process:
            ...
            if vision_embeds is not None:
                ...
                cp_local_vision_idx = self._cp_local_vision_embed_indices(vision_embeds, input_ids, packed_seq_params)
            ...
            if cp_local_vision_idx is not None and deepstack_feature_lists is not None:
                deepstack_feature_lists = [
                    embed.index_select(0, cp_local_vision_idx) for embed in deepstack_feature_lists
                ]
